...
df_filt <- summary_long %>% filter(extract == "Loosely Bound")
# df_filt <- summary_long %>% filter(extract == "Tightly Bound")

p <- ggplot(df_filt, aes(x = exp_group, y = avg, fill = assay)) +
  
  # Concentration Plots
  geom_col(
    data = subset(df_filt, y_label == "\u00b5g/mgTSS"),
    position = "dodge",
    width = 0.8
  ) +
  geom_errorbar(
  ...
  # PN/PS plots
  geom_col(
    data = subset(df_filt, y_label == "PN/PS"),
    width = 0.5
  ) +
  geom_errorbar(
    data = subset(df_filt, y_label == "PN/PS"),
  ...
  force_panelsizes(cols = c(0.35, 1), rows = c(1, 0.4)) +
...
fname_out <- "./figures/EPS_LB_big.png"
# fname_out <- "./figures/EPS_TB_big.png"
